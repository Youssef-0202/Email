Feature: Utilisateur
  ...
  Background:
    ...
    * def putBody = read('UtilisateurUpdate.json')
    * def objectSchema = read('UtilisateurSchema.json')
  ...
  Scenario Outline: Find All items (204 & 200)
    ...
    * match res == { pass: true, message: null }
    ...
    Examples:
      ...
      | 200          | ''            | GET    | ["each response contains objectSchema "," response[0].username == postBody.username"] |
